...
const getAuthorizedClient = () => new Promise((res, rej) => {
...
const getAllTimeData = analytics => new Promise((res, rej) => {
...
const getLastMonthData = analytics => new Promise((res, rej) => {
  analytics.data.ga.get({
...
const processRows = (allTimeRows, last30Rows) => {
  fs.readdir('./pages/posts/', (err, files) => {
    _(files).filter(file => _.includes(file, '.md')).forEach(file => {
      ...
      let analyticsCountLastMonth = getPageViewCounts(attributes.path, last30Rows);
      attributes.pageViews = analyticsCount;
      attributes.last30pageViews = analyticsCountLastMonth;
      let printedAttributes = _.map(attributes, (value, key) => `${key}: "${value}"`).join('\n');
      let newContents = `---\n${printedAttributes}\n---\n${body}`;
...
getAuthorizedClient().then(analytics => {
  ...
  Promise.all(promises).then(([allTimeRows, last30Rows]) => processRows(allTimeRows, last30Rows));
});
